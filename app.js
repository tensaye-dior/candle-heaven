(() => {
  ...
  const toast = message => { const el=$("[data-toast]"); if(!el) return; el.textContent=message; el.classList.add("show"); setTimeout(()=>el.classList.remove("show"),2200); };
  ...
  function renderProducts(){
    ...
    const empty=$("[data-empty]"); if(empty) empty.hidden=shown.length>0;
  ...
  function renderCart(){
    ...
    const empty=cart.length===0; const e=$("[data-cart-empty]"),s=$("[data-cart-summary]"),t=$("[data-cart-total]"); if(e)e.hidden=!empty;if(s)s.hidden=empty;if(t)t.textContent=money(total());
  ...
  function setupProductPage(){
    const root=$("[data-product-page]"); if(!root)return; const p=productById(new URLSearchParams(location.search).get("id"))||products[0]; if(!p)return;
    document.title=`${p.name} — Candle Heaven`; root.innerHTML=`<div class="detail-image"><img src="${p.image}" alt="${p.name} candle"></div><div class="detail-copy"><p class="eyebrow">${p.badge||"CANDLE HEAVEN AUSTRALIA"}</p><h1>${p.name}</h1><p class="detail-notes">${p.notes}</p><strong class="detail-price">${money(p.price)}</strong><p class="detail-description">${p.description}</p><dl><div><dt>Size</dt><dd>${p.size}</dd></div><div><dt>Burn time</dt><dd>${p.burn}</dd></div><div><dt>Wax</dt><dd>Natural soy</dd></div></dl><div class="detail-buy"><label>Quantity<select data-detail-qty><option>1</option><option>2</option><option>3</option><option>4</option></select></label><button class="button gold" data-detail-add>Add to bag</button></div><p class="shipping-note">Australia-wide delivery. International shipping options may vary by destination.</p><p class="shipping-note"><a href="policies.html">Shipping, returns & candle safety</a></p></div>`;
    $("[data-detail-add]").onclick=()=>{add(p.id,Number($("[data-detail-qty]").value));$("[data-cart-dialog]")?.showModal();};
  }

  function checkout(){ $("[data-cart-dialog]")?.close(); const t=$("[data-checkout-total]"); if(t)t.textContent=money(total()); $("[data-checkout-dialog]")?.showModal(); }
  function submitOrder(e){
    e.preventDefault(); const data=Object.fromEntries(new FormData(e.currentTarget));
    const order={customer:data,cart,total:total(),createdAt:new Date().toISOString()};
    localStorage.setItem("candleHeavenPendingOrder",JSON.stringify(order));
    if(config.squarePaymentLink){ location.href=config.squarePaymentLink; return; }
    if(config.stripePaymentLink){ location.href=config.stripePaymentLink; return; }
  ...
  $$('[data-facebook]').forEach(a=>a.href=config.facebook); $$('[data-instagram]').forEach(a=>a.href=config.instagram); $$('[data-email]').forEach(a=>a.href=`mailto:${config.email}`); $$('[data-phone]').forEach(a=>a.href=`tel:+${config.phone}`);
